...
- **Workflow**: never commit/push automatically, PRD (docs/prd.md) is the source of truth for scope. If a PRD decision gets overridden, flag it explicitly and, once confirmed, update docs/prd.md in the same piece of work — don't let it fall behind.
- **Visual identity**: **[PLACEHOLDER — see CLAUDE.md's Visual identity section for this product's actual color tokens and component treatment.]** Whimsy proposals must not introduce off-brand color or effects to "pop" once that identity is defined.
- **Important tension to hold — [PLACEHOLDER]**: define whether this product needs to read as serious/trustworthy infrastructure (favoring subtle, functional delight — honest microcopy, restrained micro-interactions) versus a consumer product where more overt gamification, confetti, or easter eggs are appropriate. Don't assume either register by default — confirm it with the product's own PRD/CLAUDE.md before proposing whimsy that could undermine its credibility goals.
